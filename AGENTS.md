...
- **SP Workout Samples** (rebuilt 2026-08, second pass — Frame Fitness "Sample Our Expert-Led Pilates Workouts" clone): `sections/sensol-product-video-carousel.liquid` — coverflow CAROUSEL (not grid): centered active 16:9 slide, side slides peek with scale(1-0.2n)/opacity(1-0.5n) falloff, prev/next chevron arrows + pagination dots, identical behavior PC/mobile (no drag — arrows/dots only, like Frame). Slide anatomy = Frame's: `<video poster><source>` + 64px circle play button + `<h4>` name + `<p>` description paragraph. Light cream bg (#F7F4F0) — sits between dark feature and light specs on rs03-2. Block settings: `video` (Shopify upload) OR `video_url` (mp4 URL) + `poster_image` (image picker) OR `poster_asset` (theme-asset filename resolved via `asset_url` — the ONLY way to reference machine-generated posters without Admin API Files access) + `title` + `text` (description). **Renders NOTHING when no video set** (has_video guard wraps style+markup+script; schema stays at file root level — never wrap {% schema %} in an if-block).
