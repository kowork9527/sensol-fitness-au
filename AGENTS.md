...
- **SP Workout Samples** (rebuilt 2026-08 from old "SP Video Carousel"): `sections/sensol-product-video-carousel.liquid` — 16:9 landscape grid, 1-3 (max 4) video cards, click-to-play inline with controls, identical layout PC/mobile. Block settings: `video` (Shopify upload, preferred) OR `video_url` (external mp4 fallback) + optional `poster_image` (external mode only) + `title` + `meta` (duration/level). **Renders NOTHING when no video set** (has_video guard wraps style+markup+script; schema stays at file root level — never wrap {% schema %} in an if-block).
- Video inventory truth (2026-08): store Files contain 4 real demo videos on Shopify CDN (`cdn.shopify.com/videos/c/o/v/*.mp4`) — product media has ZERO videos (GraphQL media query missed them; they live in store Files, referenced via URL settings). RS03 (`product.sensol-rs03-2.json`, section key `video` NOT `sp_video` — the 2026-08 removal pass missed it because it keyed on "sp_video") carries all 4 (Full Showcase/Arm Range Demo/Pilates Flow/Strength Training). Other 6 templates have DORMANT `sp_video` entries — invisible until merchant adds video blocks via theme editor.
